...
class Sat3DScene(ThreeDScene):
    def create_scalloped_cap(
            ...
            cap = Surface(
                lambda u, v: (
                    ...
                    * np.array([
                        np.sin(
                            min(u * phi_cap, scalloped_phi_cut(v))
                        ) * np.cos(v),
                        np.sin(
                            min(u * phi_cap, scalloped_phi_cut(v))
                        ) * np.sin(v),
                ...
                u_range=[0, 1],
                v_range=[0, TAU],
                resolution=(mesh_res[0], num_scallops*10,))
    ...
    def construct(self):
        ...
        cap2.set_stroke(GOLD_E, width=1, opacity = .2)
        ...
        for i in range(num_scallops*2):
            ...
            dot2 = Dot(p_world2, radius=0.2, color=RED).shift(IN * shift_net + (height*1)*OUT)
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                x_pos = col * (rect_width + horizontal_buffer) - (cols - 1) * (rect_width + horizontal_buffer) / 2
                y_pos = -row * (rect_height + vertical_buffer)
                
                # Move to position, orthogonal to z-axis (facing OUT)
                rect.move_to([x_pos, y_pos, 0])
                rect.z_index = 1
                rect.set_color(GREY_E).set_fill(opacity=1)#.set_sheen(0.8, direction=UR).set_sheen_color(BLUE_B)
        ...
        col1_x = 0 * (rect_width + horizontal_buffer) - (cols - 1) * (rect_width + horizontal_buffer) / 2
        col2_x = 1 * (rect_width + horizontal_buffer) - (cols - 1) * (rect_width + horizontal_buffer) / 2
        ...
        box_height = .75  # Adjust as needed
        box_depth = 1.75
        box = Prism(dimensions=[box_width, box_height, box_depth]).set_fill(GREY_E, opacity=1)
        box.z_index = 0
        # Position box centered between the two rectangles, touching their bottom
        center_x = (col1_x + col2_x) / 2
        box_y = bottom_row_y - rect_height/2 - box_height/2  # Bottom of rectangles minus half box height
        ...
        self.set_camera_orientation(phi=75*DEGREES, theta=0*DEGREES, zoom = .5)
